...
| Week&nbsp;10 | Final repository polish for the team project, GitHub portfolio preparation, commit-history review, cleanup of merged local branches, GitHub profile repository pinning, profile README review, final presentation outlining, technical backend case-study writing, architecture and engineering-challenge documentation, verified backend testing metrics, CV bullet preparation, LinkedIn post drafting, presentation rehearsal planning, live demo preparation, technical Q&A preparation, fallback demo planning, final presentation timing preparation, pre-presentation checklist preparation, live presentation delivery planning, evidence-based Q&A handling, post-presentation gap follow-up planning, final evaluation review, capstone code-review preparation, certification outcome review, final feedback preparation, and 10-week program reflection | [View Week 10](./BinX%20Internship/Week%2010) |
...
- Inspecting generated SQL using `LogTo`
- Development diagnostics using `EnableSensitiveDataLogging`
- Measuring actual database query counts
...
- Running the final Sprint 4 Review against the Definition of Done
- Demonstrating the live Railway API using Postman
- Verifying the passing GitHub Actions CI/CD pipeline
- Confirming the active Railway deployment
- Reviewing the final Sprint 4 backlog status
- Confirming that all Definition of Done items passed
- Writing the Sprint 4 Retrospective
- Recording what went well during testing, documentation, and deployment
- Identifying deployment configuration and production documentation improvements
- Defining Week 10 presentation preparation as the next action item
- Closing Phase 3 and preparing for the final project presentation
...
### Sprint 3 Planning and Database Performance

- Starting Phase 3 Sprint 3 for the Cardiac Patient Monitoring System API
- Defining a database-performance-focused Sprint 3 goal
- Carrying forward the Sprint 2 retrospective improvement action
- Creating and organizing the Sprint 3 backlog
- Defining measurable query-performance targets
- Enabling Entity Framework Core SQL query logging
- Using `LogTo` with `LogLevel.Information`
- Using `EnableSensitiveDataLogging` for development diagnostics
- Inspecting the actual SQL generated by Entity Framework Core
- Seeding realistic test data for performance analysis
- Preparing at least 50 VitalSign records
- Preparing at least 50 Medication records
- Preparing at least 50 Appointment records
- Measuring SQL query counts for important list endpoints
- Measuring `GET /api/Appointments`
- Confirming two fixed SQL queries for paginated appointment retrieval
- Measuring `GET /api/VitalSigns`
- Confirming one SQL query for VitalSign list retrieval
- Measuring `GET /api/Medications`
- Confirming one SQL query for Medication list retrieval
- Reviewing existing list endpoints for N+1 query patterns
- Understanding the N+1 query problem
- Recognizing the `1 + N` repeated-query pattern
- Confirming that the tested existing list endpoints do not contain an N+1 issue
- Reviewing service and repository query behavior
- Confirming that EF Core lazy-loading proxies are not used in the project
- Creating a dedicated diagnostic endpoint to reproduce an N+1 query pattern
- Executing a related Patient query once for each VitalSign record
- Confirming 51 SQL queries for 50 VitalSign records
- Establishing a measurable N+1 performance baseline
- Recording response time as an observational performance metric
...
- Running the final Definition of Done audit against the actual completed project
- Verifying the live Railway API through the public URL
- Confirming Swagger/OpenAPI documentation completeness
- Confirming at least one Postman test for every endpoint
- Verifying the ERD and EF Core migrations
- Reviewing the main project README against the final documentation requirements
- Adding the required production environment variable documentation
- Adding direct Swagger and OpenAPI documentation paths
- Verifying the latest GitHub Actions CI/CD pipeline
- Building the project successfully with zero compiler warnings
- Closing all remaining Definition of Done gaps
- Completing the Sprint 4 Review
- Completing the Sprint 4 Retrospective
- Closing Phase 3
- Preparing the project for the Week 10 final presentation
...
### Final Repository & Portfolio Preparation

- Reviewing a team repository as a professional portfolio artifact
- Searching for leftover debug code and temporary development artifacts
- Reviewing `TODO` and `FIXME` comments
- Reviewing commented-out experimental code
- Cleaning merged local Git branches
- Preserving unmerged team branches that may still be in active use
- Verifying a clean Git working tree
- Reviewing complete Git commit history
- Evaluating commit messages as part of professional engineering practice
- Reviewing Conventional Commit-style prefixes such as `feat:`, `fix:`, and `docs:`
- Pinning the team project repository to a GitHub profile
- Reviewing an existing GitHub profile README
...
- Planning a complete timed rehearsal for the final project presentation
- Practicing presentation delivery out loud instead of reading directly from slides
- Measuring the complete presentation duration
- Adjusting presentation pacing based on rehearsal results
- Planning the final live project demo
- Including the live demo as part of the timed presentation rehearsal
- Preparing for network, hosting, and service-related demo issues
- Preparing a backup demo recording as a fallback plan
- Anticipating likely technical questions before the final discussion
- Preparing clear and honest technical answers
- Explaining ASP.NET Core and FastAPI architectural responsibilities
- Explaining backend-to-FastAPI response validation
- Explaining how the service integration was verified through automated and integration testing
- Discussing future scalability based on measured bottlenecks rather than assumptions
- Identifying possible future improvements without presenting unimplemented work as completed
- Preparing a second rehearsal and mock Q&A plan
- Planning final presentation timing verification after the project and presentation are fully completed
- Documenting incomplete rehearsal and demo activities as preparation plans while the team project remains under active development

### Final Presentation Delivery Preparation

- Preparing a pre-presentation checklist before the final BinX Tech staff session
- Verifying that the deployed API is responsive before the presentation begins
- Planning a warm-up request when the hosting service has been idle
- Verifying that the presentation file opens correctly
- Confirming that Postman is ready with the correct collection and required variables
- Testing the main demo request before the presentation starts
- Planning a meaningful live API demo scenario for the Trip Planning Platform
- Narrating architectural reasoning while the live demo is running
- Preparing a backup demo recording for possible technical issues
- Preparing an evidence-based approach for live technical Q&A
- Answering technical questions honestly without unsupported guesses
- Explaining architectural decisions using actual project behavior, testing evidence, and known trade-offs
- Preparing a structure for documenting questions asked during the final presentation
- Preparing a structure for recording genuine technical or presentation gaps revealed during the session
- Planning follow-up actions for important gaps after the presentation
- Keeping the final presentation, live demo, and Q&A documented as planned activities until they are actually completed

### Final Evaluation, Code Review & Certification Preparation

- Reviewing the four weighted evaluation criteria used for the final program grade
- Confirming Technical Skill Development as `40%` of the final grade
- Confirming Sprint / Deliverable Quality as `30%`
- Confirming Professionalism & Consistency as `20%`
- Confirming the Final Presentation as `10%`
- Preparing for the final code review of the complete capstone project
- Reviewing architecture, code quality, automated testing, security practices, error handling, and service integration as final review areas
- Preparing a structure for documenting official evaluation scores after they are received
- Reviewing the minimum direct passing grade of `70/100`
- Reviewing the Outstanding, Pass, Conditional Pass, and Does Not Pass outcomes
- Preparing a structure for documenting final mentor and review-panel feedback
- Preparing a structure for documenting the final certification outcome and next steps
- Keeping official scores, feedback, and certification results undocumented until they are formally received
- Writing a personal reflection on the complete 10-week internship experience
- Reflecting on evidence-based engineering, testing before shipping, secure development, teamwork, code quality, and continuous improvement
- Recognizing the end of the internship as the beginning of continued professional engineering development
